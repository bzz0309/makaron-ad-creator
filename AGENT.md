...
- Compose Before/After locally and deterministically at exactly `1080×1920`: black background, one shared rendered image height, full-frame aspect-ratio preservation, exactly 10px between the actual rendered image boxes, at least 40px symmetric outer margins, and same-baseline labels centered under their actual images 35px below the image edge. Never send the comparison node to a model. Its pixel-level QC must verify equal rendered height/top/bottom, actual image gap, outer-margin symmetry, label centering, vertical group centering, and black pixels outside the image/label regions.
