...
默认录屏节点不再由本地脚本设计。CLI 调用 Makaron 公开可选的 `screen-demo`，把对应语言的 v5 iOS UI 基准图作为视觉参考，交由 Makaron 生成约 4 秒的 Remotion design。CLI 先请求 Makaron 云端 materialize；若当前账号的 composition export 返回 `403 Forbidden`，只在本机编码 Makaron 返回的同一份受限 design，不会改写 UI、时间线或内容。内置但不可公开选择的 `synthetic-screen-recording` 不会被 CLI 直接调用；旧 v5 本地 workflow renderer 不在当前 DAG 中。
...
- Synthetic workflow design is produced by Makaron's public `screen-demo` Skill with supplied v5 iOS UI frames as references. It is never labeled as genuine screen recording; the legacy local v5 renderer is not part of the default DAG. If Makaron composition export is forbidden, the CLI may encode the unchanged bounded Makaron Remotion design locally.
