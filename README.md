...
- One instrumental BGM is generated per campaign with `makaron music create`, reused for every selected locale, mixed at `0.14` in narration gaps, and ducked to `0.08` around measured Seed Audio captions so the foreground TTS stays clear through the final CTA frame.
