...
- Replaced the master DAG's local v5 workflow renderer with Makaron's public `screen-demo`; locale-correct v5 iOS baseline frames are passed as visual references. The CLI does not invoke the non-selectable internal `synthetic-screen-recording` capability and forbids local Remotion fallback for this four-second node.
