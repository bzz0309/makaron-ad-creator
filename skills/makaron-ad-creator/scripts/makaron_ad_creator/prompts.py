...
META SAFE ZONE: treat the insets as canvas-relative ratios, not fixed 1080p pixel coordinates. On the required {output['width']}x{output['height']} composition they resolve to x={safe['left_px']}..{output['width'] - safe['right_px']} and y={safe['top_px']}..{output['height'] - safe['bottom_px']}, with captions at y={safe['caption_top_px']} or lower. If any runtime preview uses another 9:16 size, recompute every pixel inset from the ratios before layout so captions and faces never drift or clip. The Meta profile overrides the older 140px top-caption convention because 140px lies under account UI.
...
Burn exactly one synchronized subtitle set for every spoken line: white text with black stroke/outline, no black box or background bar, horizontally centered inside the safe content region with textAlign:'center' and centered flex alignment, never left-aligned or shifted toward either rail. Use maximum two lines and at most {safe['max_characters_per_line']} visible characters per line with measured automatic wrapping that never leaves the safe zone. Caption text/display must not contain literal backslash-n or hard-coded newline characters; let the renderer wrap from measured width. English subtitles are uppercase; Japanese and Cantonese retain natural case. Do not add duplicate subtitles, title-card captions, or overlapping text layers. Measure text width before rendering and reduce font size or wrap when necessary.
