...
- `audio.tts_volume_by_locale`: optional foreground voiceover-only gain map for `en`, `ja`, and `yue`; defaults to `1.35` and accepts `0.5–2.0` without changing BGM volume.
...
- `audio.bgm_volume`: Remotion BGM volume outside narration, default `0.14` and maximum `0.5`.
- `audio.bgm_ducking`: required caption-timed Seed Audio ducking, default `{enabled:true, ducked_volume:0.08, attack_ms:80, release_ms:240}`. BGM must duck to this lower volume around every measured voice interval and recover only in narration gaps.
