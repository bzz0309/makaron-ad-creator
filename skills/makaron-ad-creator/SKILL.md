...
4. Generate a neutral ordinary Before image. Invoke the target Skill exactly once for one continuous effect source, preferring about eight seconds. Derive Hook from its opening range and Result from its later non-overlapping range; never make a second generative Hook request. In the bound Makaron project, analyze the complete source and export its strongest stable decoded frame as the exact After—never use a fixed percentage and never redraw it. Then ask Makaron to compose the locked Before and After pixels on the black side-by-side canvas.
