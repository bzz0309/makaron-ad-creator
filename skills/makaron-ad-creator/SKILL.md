...
5. For each selected locale, invoke Makaron's public `screen-demo` Skill in the bound project. Supply the authorized input plus the bundled v5 iOS `home-top`, `home-target`, and `detail` UI frames as visual references. Makaron creates the synthetic four-second localized workflow through its Remotion runtime: English uses `en`, Japanese uses `ja`, and Cantonese uses `zh-Hant`. Do not invoke the non-selectable internal `synthetic-screen-recording` capability directly, and do not use the legacy local renderer in the default DAG.
