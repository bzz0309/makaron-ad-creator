...
3. Generate five-line culturally adapted script JSON for only the selected ad locales. Keep one personal first-person user/creator perspective across all five beats: their photo, their Makaron actions, and their result. Never switch to detached third-person narration; lines 3–4 describe actions already taken rather than commands to the viewer.
4. Generate a neutral ordinary Before image. Invoke the target Skill exactly once for one continuous effect source, preferring about eight seconds. Derive Hook from its opening range and Result from its later non-overlapping range; never make a second generative Hook request. In the bound Makaron project, analyze the complete source and export its strongest stable decoded frame as the exact After—never use a fixed percentage and never redraw it. Compose Before/After deterministically in the CLI, never through a model: fixed 1080×1920 pure-black canvas, one shared `common_h` calculated from both source aspect ratios, complete aspect-preserved images with exactly 10px between their actual rendered boxes, and no crop/stretch/cover-fit/redraw. Horizontally center the combined image pair with at least 40px black on both outside edges and no more than 1px left/right margin difference. Center each label on its actual image, put `BEFORE` and `AFTER` on one baseline with each label's typographic top 35px below the actual image bottom, and vertically center the complete images-plus-labels group. Require pixel-level layout QC before the node passes.
...
The five fixed final beats are: Hook extracted from the target-Skill effect source → simultaneous Before/After comparison image → truthful localized v5 workflow video → later non-overlapping Result from the same effect source → fixed Makaron Logo CTA video. Keep the same order and timing bounds across locales; allow exact beat lengths to adapt to measured speech. TTS defaults to a natural energetic young-adult female Seed Audio voice at foreground gain `1.35` and must finish before CTA. Caption line 1 belongs to Hook, line 2 to comparison, lines 3–4 to workflow, and line 5 to result; no line may cross its scene boundary. Every source video is muted. CTA source audio is never used. One separately generated instrumental BGM is looped continuously from 0.0 seconds through the final CTA frame at `0.14` outside narration and ducked to `0.08` from 80ms before through 240ms after every measured TTS/caption interval, with only a gentle final fade. On Meta Reels use caption top `200/1920`, bottom `340/1920`, left `90/1080`, and right `180/1080`; derive pixel values from the actual composition. The top caption area is reserved only for copy so it clears faces; keep CTA and other key UI out of it. Captions use white text, black outline, no bar, horizontal centering, one-line-first responsive sizing, and at most 32 visible characters before a two-line fallback. When two lines are necessary, balance their measured widths; for English with six or more words, keep at least three words on each line and never leave a one- or two-word orphan line. Do not insert hard line breaks or the literal characters `\\n`; layout wrapping belongs to the renderer inside the safe content width.
